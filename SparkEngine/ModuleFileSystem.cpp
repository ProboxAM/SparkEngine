...
ModuleFileSystem::ModuleFileSystem(const char* game_path) : Module(true)
{
	...
	const char* dirs[] = {
		...
		LIBRARY_MATERIAL_FOLDER, LIBRARY_SCENE_FOLDER, LIBRARY_MODEL_FOLDER, LIBRARY_BONE_FOLDER,
		LIBRARY_TEXTURES_FOLDER, LIBRARY_ANIMATION_FOLDER, LIBRARY_STATE_MACHINE_FOLDER, LIBRARY_ANIM_CONTROLLER_FOLDER
	};
...
void ModuleFileSystem::GetFilesFiltered(const char* directory, std::vector<std::string> & file_list, std::string filter)
{
	char **rc = PHYSFS_enumerateFiles(directory);
	char **i;
...
void ModuleFileSystem::GetFilesOfExtension(const char* directory, std::vector<std::string> & file_list, std::string extension)
{
	char **rc = PHYSFS_enumerateFiles(directory);
	char **i;
	...
	for (i = rc; *i != nullptr; i++)
	{
		if (!PHYSFS_isDirectory((dir + *i).c_str()) && HasExtension((*i), extension))
			file_list.push_back(*i);
	}
...
std::string ModuleFileSystem::GetUniqueFile(std::string wanted_file)
{
	std::string ret;
